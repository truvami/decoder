...
import (
	...
	ts2 "github.com/truvami/decoder/pkg/decoder/ts2/v1"
...
var ts2DevEui string

func init() {
	ts2Cmd.Flags().StringVar(&ts2DevEui, "dev-eui", "", "DevEUI of the originator device.\nThis is only required for loracloud solver.")
	rootCmd.AddCommand(ts2Cmd)
}

var ts2Cmd = &cobra.Command{
	Use:   "ts2 [port] [payload]",
	Short: "decode ts2 payloads",
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		logger.Logger.Debug("initializing ts2 decoder")
		d := ts2.NewTS2v1Decoder(ctx, solver, logger.Logger, ts2.WithSkipValidation(SkipValidation))
		...
		ctx = context.WithValue(ctx, decoder.DEVEUI_CONTEXT_KEY, ts2DevEui)
