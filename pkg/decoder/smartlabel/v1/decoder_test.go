...
func TestDecodeWithNoopSolver(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(fmt.Sprintf("TestPort%vWith%v", test.port, test.payload), func(t *testing.T) {
			decoder := NewSmartLabelv1Decoder(context.TODO(), solver.NoopSolver{}, zap.NewNop())
			got, err := decoder.Decode(context.TODO(), test.payload, test.port)
...
func TestPayloadTooShort(t *testing.T) {
	...
	decoder := NewSmartLabelv1Decoder(context.TODO(), solver.MockSolverV1{}, logger)
	_, err := decoder.Decode(context.TODO(), "0ff0", 1)
...
func TestPayloadTooLong(t *testing.T) {
	...
	decoder := NewSmartLabelv1Decoder(context.TODO(), solver.MockSolverV1{}, logger)
	_, err := decoder.Decode(context.TODO(), "0ff00ff00ff0", 1)
...
func TestSmartLabelv1Solver(t *testing.T) {
	tests := []struct {
		port        uint8
		payload     string
		expected    any
	...
	decoder := NewSmartLabelv1Decoder(context.TODO(), solver, zap.NewExample())
	for _, test := range tests {
