...
// Port 212: WiFi localization triggered by rotation state change (non-moving).
// Same binary format as port 200 (timestamped WiFi) but rotation-triggered, not buffered.
//
...
const (
	Port212HeaderLength      = 5 // minimum payload bytes (4B timestamp + 1B version)
	Port212VersionIndex      = 4 // byte offset of the version field
	Port212Version1     byte = 0x00
	Port212Version2     byte = 0x01
)

type Port212Payload struct {
	Timestamp time.Time `json:"timestamp"`
	Version   byte      `json:"version" validate:"gte=0,lte=1"`
	Moving    bool      `json:"moving"` // Always false for Port 212
...
var _ decoder.UplinkFeatureWiFi = &Port212Payload{}
var _ decoder.UplinkFeatureMoving = &Port212Payload{}
var _ decoder.UplinkFeatureTimestamp = &Port212Payload{}

func (p Port212Payload) GetTimestamp() *time.Time {
	return &p.Timestamp
}

func (p Port212Payload) GetAccessPoints() []decoder.AccessPoint {
...
// Port 212 is the non-moving (steady) rotation-triggered variant.
func (p Port212Payload) IsMoving() bool {
	return false
}
