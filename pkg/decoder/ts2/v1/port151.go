...
// EFR32BG22 EMU_RSTCAUSE bit masks reported by the TS2 firmware.
...
const (
	ts2ResetCauseMask   uint32 = 0x7FFFFFFF
	ts2ResetCausePOR    uint32 = 0x00000001
	ts2ResetCausePIN    uint32 = 0x00000002
	ts2ResetCauseWDOG0  uint32 = 0x00000008
	ts2ResetCauseSYSREQ uint32 = 0x00000040
)
...
func (p Port151Payload) GetResetReason() decoder.ResetReason {
	...
	cause := *p.ResetCause & ts2ResetCauseMask
	if cause == 0 {
		return decoder.ResetReasonUnknown
	}

	switch {
	case cause&ts2ResetCausePOR != 0:
		return decoder.ResetReasonPowerReset
	case cause&ts2ResetCausePIN != 0:
		return decoder.ResetReasonPinReset
	case cause&ts2ResetCauseWDOG0 != 0:
		return decoder.ResetReasonWatchdog
	case cause&ts2ResetCauseSYSREQ != 0:
...
// DataRateFromUint8 converts a uint8 data rate value to the corresponding TS2 DataRate enum.
