...
type Option func(*TS2v1Decoder)

type TS2v1Decoder struct {
	skipValidation bool
	logger         *zap.Logger
...
func NewTS2v1Decoder(ctx context.Context, solver solver.SolverV1, logger *zap.Logger, options ...Option) decoder.Decoder {
	if solver == nil {
		logger.Panic("solver cannot be nil", zap.String("decoder", "TS2v1Decoder"))
	}

	ts2v1Decoder := &TS2v1Decoder{
		logger:         logger,
		solver:         solver,
		fallbackSolver: nil,
	}
...
func (t TS2v1Decoder) getConfig(port uint8, payload []byte) (common.PayloadConfig, error) {
	switch port {
	case 10:
...
*/
func (t TS2v1Decoder) Decode(ctx context.Context, data string, port uint8) (*decoder.DecodedUplink, error) {
		...
		if t.v2Solver != nil {
			...
			if err != nil {
				if t.fallbackV2Solver == nil {
					ts2DecoderSolverFailedCounter.Inc()
				...
				ts2DecoderSuccessfullyUsedFallbackSolverCounter.Inc()
			}
			return uplink, nil
		}
